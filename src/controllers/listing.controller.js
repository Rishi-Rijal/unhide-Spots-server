...
const getListingFiltered = AsyncHandler(async (req, res) => {
  try {
    ...
    if (c) {
      if (sort === "newest") {
        cursorMatch.$or = [
          ...
          { createdAt: new Date(c.createdAt), _id: { $lt: new mongoose.Types.ObjectId.createFromHexString(c._id) } },
        ];
      } else if (sort === "rating_desc") {
        cursorMatch.$or = [
          { averageRating: { $lt: c.averageRating } },
          { averageRating: c.averageRating, createdAt: { $lt: new Date(c.createdAt) } },
          { averageRating: c.averageRating, createdAt: new Date(c.createdAt), _id: { $lt: new mongoose.Types.ObjectId.createFromHexString(c._id) } },
        ];
        ...
        cursorMatch.$or = [
          ...
          { likesCount: c.likesCount, createdAt: new Date(c.createdAt), _id: { $lt: new mongoose.Types.ObjectId.createFromHexString(c._id) } },
        ];
        ...
        cursorMatch.$or = [
          ...
          { distanceMeters: c.distanceMeters, averageRating: (c.averageRating ?? 0), createdAt: new Date(c.createdAt), _id: { $lt: new mongoose.Types.ObjectId.createFromHexString(c._id) } },
        ];
      }
    }
    ...
    if (sort === "distance") {
      ...
      if (distanceKm > 0) geo.maxDistance = distanceKm * 1000;
      pipeline.push({ $geoNear: { ...geo, query: match } });   // pre-filter for speed
    ...
    pipeline.push({
      $project: {
        ...
        likesCount: 1,
        createdAt: 1,
        ...(sort === "distance" ? { distanceMeters: 1 } : {}),
      },
    });
    ...
    if (hasNextPage && data.length) {
      ...
      if (sort === "newest") {
        ...
        nextCursor = encode({ averageRating: last.averageRating ?? 0, createdAt: last.createdAt, _id: last._id });
      } else if (sort === "likes_desc" || sort === "likes_asc") {
        nextCursor = encode({ likesCount: last.likesCount ?? 0, createdAt: last.createdAt, _id: last._id });
      } else {
        nextCursor = encode({
